        ...
        function generateBracket(nodeData, index) {
            ...
            g.selectAll('g')
                ...
                .style("fill", function (d) {
                    if (d.data.status > 1) {
                        var newTexture = textures.lines()
                            .orientation("6/8").size(10).strokeWidth(4).background(d.data.color).stroke(shadeColor(d.data.color, 0.3));
                        g.call(newTexture);
                        return newTexture.url();
                    }
                    if (d.data.status === 1) {
                        return shadeColor(d.data.color, 0.1);
                    }
                    if (d.data.status === 0) {
                        return 'gray';
                    }
